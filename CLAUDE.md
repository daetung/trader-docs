...
Label search range:    t bar open → last_bar(date) close or the
                       execution.max_hold_bars'th valid bar close (whichever
                       first). last_bar() is PER DATE — utils.md — and is 12:59
                       on an early-close day, not 15:59
