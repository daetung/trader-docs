"""[실시간]해외주식 호가 [V61] — standalone WebSocket 예제.
...
해외주식(미국) 실시간 호가 API 입니다. ※ 해외주식(미국) 무료실시간시세 신청을 하지 않을 경우 실시간 시세를 수신 하실 수 없습니다. (V10,V11 지연시세는 별도 신청없이 사용 가능하십니다.) ※ 실시간무료시세(0분) 신청방법 HTS : [7325] 해외주식 실시간 시세 신청 MTS : 해외주식 > 서비스신청 > 실시간시세신청 ※ 무료 실시간 시세는 전체 시세에 비해 50% 수준의 체결 데이터를 제공합니다. ※ 무료 실시간 시세 자동결제 신청 시 당월 거래가 없거나 말일에 보유한 미국주...
...
실행:
    ...
    python ov_stock_realtime/ov_stock_realtime_orderbook.py
    # examples/ov_stock_realtime/ 폴더에서 실행하는 경우:
    python ov_stock_realtime_orderbook.py
