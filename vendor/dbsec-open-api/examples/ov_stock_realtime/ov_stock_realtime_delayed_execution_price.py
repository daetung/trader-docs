"""[실시간]해외주식 지연체결가 [V10] — standalone WebSocket 예제.
...
해외주식(미국) 지연 체결가 API 입니다. ※ 15분 지연된 실시간 시세를 받아보실 수 있습니다. ※ 해외주식(미국) 무료실시간시세 신청을 하지 않을 경우 실시간 시세를 수신 하실 수 없습니다. (V10,V11 지연시세는 별도 신청없이 사용 가능하십니다.) ※ 실시간무료시세(0분) 신청방법 HTS : [7325] 해외주식 실시간 시세 신청 MTS : 해외주식 > 서비스신청 > 실시간시세신청
...
실행:
    ...
    python ov_stock_realtime/ov_stock_realtime_delayed_execution_price.py
    # examples/ov_stock_realtime/ 폴더에서 실행하는 경우:
    python ov_stock_realtime_delayed_execution_price.py
